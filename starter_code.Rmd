...
set.seed(10)
sampledata<-clean %>%
  initial_split(prop=.01)

training_tbl <- sampledata %>% training() 

# Can also do training(sampledata)

test_tbl  <- sampledata %>% testing()
```
...
logistic_reg() %>%
  set_engine("glm") %>%
  fit(data=training_tbl,dvote ~ gender + party + state + office) %>%
  predict(new_data=training_tbl) %>%
  bind_cols(training_tbl) %>%
  metrics(truth=dvote, estimate=.pred_class) %>%
  gt() %>% 
    tab_header("Model 2 Evaluation on Training Data")

# Smaller the data the more accurate our prediction will be
```


```{r}
# How good does a model (which has been estimated on the sample data) work on 
# the testing data, that is, on data which was not seen when you were estimating
